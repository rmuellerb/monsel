...
static struct argp_option options[] =
{
    ...
    {"check", 'c', 0, 0, "Switch on model checking before run (default: no)"},
...
int check_model_files(char* basefile, int changes, int verbose)
{
    ...
    // First read basefile
    NetworkModel model;
    if(read_base_file(&model, basefile) == -1)
    {
        if(verbose)
    ...
    for(i=0; i<=changes; i++)
    {
        ...
        if(!fname)
            return -1;
        if(read_diff_file(&model, fname) == -1)
        {
            if(verbose)
                printf("ERROR: Reading file \"%s\" failed!\n", fname);
            return -1;
 ...
 * Returns in case we have the maximum number of allowed evaluations is hit.
 */
long localsearch(Individual *ind, int k, NetworkModel *model, Fitness *fitvals, long nevals, long max_evals, char* inputfname)
{
    if(ind->size < k)
    {
...
{
    printf("WARNING: This is the continuous optimization version of monitor selection.\nFor the basic one, use monsel instead of monsel_continuous!\n");
    long i = 0;
    srand(time(NULL));
    ...
    {    
        ...
        if(check_model_files(params.inputfname, params.max_evals, params.verbose) == -1)
        {
    ...
    for(i=0; ; i++)
    {
        ...
        if(params.do_localsearch)
        {
            ...
            ls_nevals = nevals;
            nevals = localsearch(&pop.ind[idx], params.ls_k, &model, fitvals, nevals, params.max_evals, params.inputfname);
